...
public:
	...
	BOOL OpenPort(LPCTSTR Port,int Baudrate,int DataBits,int StopBits,int Parity,int type,LPDataArriveProc proc=NULL,DWORD userdata=0); //打开串口
	BOOL ClosePort(); //关闭串口
	...
	DWORD ReadPort(unsigned char *data,unsigned int length,int timeout_ms = 0); //读取一定长度的数据
